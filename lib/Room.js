...
export default class Room { //Ideally this should be abstract, but can be used as a wall if you want
	...
	draw(ctx, width){ //return room contents
		//Clear room
	...
	drawContents(ctx, width){
		var char_count = 0;
		var item_count = 0;
		for(var i = 0; i < this.contents.length; i++){
			if(this.contents[i] instanceof Character){
				char_count++;
			}else{
				item_count++;
			}
	...
	drawNeighbours(ctx, width, recursion_level=0, exclude=null, draw_contents=false){
